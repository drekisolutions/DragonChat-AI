      ...
      <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <p className="font-cinzel" style={{ color: 'var(--text)', fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {branding?.businessName || 'Your Business'}
            </p>
            {branding?.tagline && (
              <p style={{ color: 'var(--muted)', fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 }}>{branding.tagline}</p>
            )}
          </div>
        </div>
      </div>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 4, marginBottom: 2, textDecoration: 'none', background: active ? 'var(--brand)' : 'transparent', color: active ? '#0F0F0F' : 'var(--muted)', fontSize: 13, fontWeight: active ? 600 : 400, transition: 'all 150ms ease', position: 'relative' }}>
